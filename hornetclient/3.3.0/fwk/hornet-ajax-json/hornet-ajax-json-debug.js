...
AjaxJson = {
    ...
    parseResponseData: function(oResponseText) {
        ...
        try {
            ...
            bError = true;
            Y.log("Error while parsing JSON data: " + errorParsing.message, "error", AjaxJson.NAME);
        }
        if(!data_out) {
            bError = true;
        }
        ...
        if(bError) {
            Y.log("JSON data could not be parsed: " + Y.dump(oResponseText), "error", AjaxJson.NAME);
            oParsedResponse.error = true;
        }
        else {
            Y.log("Parsed JSON data is " + Y.dump(oParsedResponse), "info", AjaxJson.NAME);
        }
